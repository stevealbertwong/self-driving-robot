#!/usr/bin/python
## run on raspberry pi 
## subscribe wheels linear 
## => angular + pid control => motor cmd + send
...
class ControlsToMotors:
  def __init__(self):
    ...
    # Parameter server (motor + pid controller config)
    ...
    # (if no PID) Publish the computed angular velocity targets
    ...
    # (if PID) Publish the computed angular velocity control command
    ...
    # Read in tangential/linear velocity targets => callback
    ...
    # PID control variables dictionary
    self.lwheel_pid = {}
    self.rwheel_pid = {}
  ...
  # ==================================================
  # Target linear velocity converts to motor commands functions
  # tangent/linear => angular + pid control => motor cmd 
  # ==================================================

  # if pid control is off
  ...
  ## PID control 
  ## TODO: IMU to do control
  ## target: wheel_angular_vel_target
  ## state: wheel_angular_vel_enc
  ## control_signal: pid(target - state)
  ## return: target_new = target + control_signal
  def pid_control(self,wheel_pid,target,state):
    ...
    ## increase in throttle as indicated by error
  ...
  ## tangent => angular + pid control => motor command => send via gopigo
  def lwheel_update(self):
    ...
    # Compute motor command using slope motor torque/angular velocity
...
def main():
  ...
  ## initialize (param server + publisher/subsriber)
  controls_to_motors = ControlsToMotors(); 

  ## subscribe to tangent => angular + pid control 
  ## => motor command => send via gopigo
